...
public class StudentManagerApplication {
    ...
    public void initialize() {
        ...
        // bind each column to its corresponding field in the Student class
    ...
    /**
     * Uses the StudentManager method, searchStudentById to find a student with an input id.
     */
    @FXML
    public void searchById() {
        ...
        try {
            ...
            Student foundStudent = studentManager.searchStudentById(id);

            if (foundStudent != null) {
                // store the search result in temporary ObservableList
                ObservableList<Student> searchResult = FXCollections.observableArrayList();
                searchResult.add(foundStudent);
            ...
            // clear the TableView by setting an empty ObservableList
            studentDisplayTable.setItems(FXCollections.observableArrayList());
            studentDisplayTable.setPlaceholder(new Label("No student with this id"));
    ...
    public void searchByName() {
        ...
        if (studentFound) {
            ...
            // display a student not found message in the table view
            studentDisplayTable.setItems(FXCollections.observableArrayList());
            showAlert(Alert.AlertType.INFORMATION, "Not Found", "Student not found.");
    ...
    public void saveData() {
        ...
        dialog.showAndWait().ifPresent(fileName -> {
            ...
            if (fileName.trim().isEmpty()) {
                ...
                // create final variable to use in lambda expression
                ...
                if (file.exists()) {
                    // show confirmation dialog if the file already exists
                    Alert confirmDialog = new Alert(Alert.AlertType.CONFIRMATION);
                    confirmDialog.setTitle("File Exists.");
                    confirmDialog.setHeaderText("A file with this name already exists.");
                    confirmDialog.setContentText("Do you wish to override it?");
                    ...
                    confirmDialog.showAndWait().ifPresent(response -> {
                        if (response == yesButton) {
                            // override the data in that file
                            studentManager.saveToFile(finalFileName, students);
                            showAlert(Alert.AlertType.INFORMATION, "Save Data", "Data saved successfully to " + finalFileName);
                        } else {
                            showAlert(Alert.AlertType.INFORMATION, "Save Cancelled", "File was not saved.");
                        }
                    });
                } else {
